...
-- Deprecated
WarMenu.debug = true
function WarMenu.SetDebugEnabled(_) end
...
local function setMenuVisible(id, visible, holdCurrentOption)
    ...
    if visible then
        ...
        elseif not holdCurrentOption then
...
local function getCurrentIndex()
    if currentMenu.currentOption <= getStyleProperty("maxOptionCountOnScreen") and
        optionCount <= getStyleProperty("maxOptionCountOnScreen") then
        return optionCount
    elseif optionCount > currentMenu.currentOption - getStyleProperty("maxOptionCountOnScreen") and
        optionCount <= currentMenu.currentOption then
...
local function drawTitle()
    ...
    if getStyleProperty("titleBackgroundSprite") then
        DrawSprite(getStyleProperty("titleBackgroundSprite").dict, getStyleProperty("titleBackgroundSprite").name, x, y,
                   getStyleProperty("width"), titleHeight, 0.0, 255, 255, 255, 255)
    else
...
local function drawSubTitle()
    ...
    if optionCount > getStyleProperty("maxOptionCountOnScreen") then
        ...
        drawText(tostring(currentMenu.currentOption) .. " / " .. tostring(optionCount),
                 getStyleProperty("x") + getStyleProperty("width"), y - buttonHeight / 2 + buttonTextYOffset)
    end
end
...
function WarMenu.ToolTip(text, width, flipHorizontal)
    ...
    setTextParams(buttonFont, getStyleProperty("textColor"), buttonScale, false, true, false, textX,
                  textX + width - (buttonTextYOffset * 2))
...
function WarMenu.SpriteButton(text, dict, name, r, g, b, a)
    ...
    DrawSprite(dict, name,
               ...
               getStyleProperty("y") + getTitleHeight() + buttonHeight + (buttonHeight * currentIndex) - spriteHeight / 2 +
                   buttonSpriteYOffset, spriteWidth, spriteHeight, 0.0, r or 255, g or 255, b or 255, a or 255)
    return pressed
end
...
function WarMenu.ComboBox(text, items, currentIndex, selectedIndex, callback)
    ...
    if itemsCount > 1 and isCurrent then
        selectedItem = ("< %s >"):format(tostring(selectedItem))
    end
